-- ============================================================
-- VOHITRA IMMOBILIER — Script SQL complet
-- À exécuter dans : Supabase → SQL Editor
-- Remplace toutes les migrations fragmentées.
...
-- 1. ENUMS
-- ============================================================
DO $$ BEGIN CREATE TYPE public.app_role       AS ENUM ('locataire', 'proprietaire', 'admin');     EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.property_type  AS ENUM ('appartement', 'maison', 'local_commercial', 'terrain'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.property_status AS ENUM ('active', 'inactive', 'pending');          EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN CREATE TYPE public.report_status  AS ENUM ('pending', 'reviewed', 'dismissed', 'removed'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- has_role / is_admin : utilisés dans les politiques RLS
...
-- Création automatique du profil + rôle au signup
...
-- Validation du contenu des annonces (pas de phone/email/url dans titre ou description)
...
-- Empêche un owner de s'auto-promouvoir premium/verified
...
-- Auto-premium si le propriétaire a un abonnement actif
...
-- Débit atomique de jetons (anti-race condition)
...
BEGIN
  ...
  UPDATE public.profiles
   ...
   WHERE id = _user_id AND tokens_balance >= _cost
...
-- Crédit de jetons (remboursements, admin)
...
-- Retourne le téléphone de visite seulement si autorisé
...
-- Nettoyage des comptes non confirmés après 30 min (appelé par pg_cron)
...
-- Wrappers pgmq pour Edge Functions (service_role uniquement)
...
CREATE INDEX IF NOT EXISTS idx_properties_premium           ON public.properties(is_premium, premium_until);
CREATE INDEX IF NOT EXISTS idx_properties_owner            ON public.properties(owner_id);
CREATE INDEX IF NOT EXISTS idx_properties_status_premium_created ON public.properties(status, is_premium DESC, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_properties_city_lower       ON public.properties(lower(city));
CREATE INDEX IF NOT EXISTS idx_properties_property_type    ON public.properties(property_type);
CREATE INDEX IF NOT EXISTS idx_properties_listing_type     ON public.properties(listing_type);
CREATE INDEX IF NOT EXISTS idx_properties_owner_status     ON public.properties(owner_id, status, created_at DESC);
ALTER TABLE public.properties ENABLE ROW LEVEL SECURITY;

-- PROPERTY CONTACTS (numéro de visite isolé, accès protégé)
...
CREATE INDEX IF NOT EXISTS idx_property_photos_property ON public.property_photos(property_id, display_order);
...
CREATE INDEX IF NOT EXISTS idx_property_views_prop_created ON public.property_views(property_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_contact_unlocks_user_property ON public.contact_unlocks(user_id, property_id);
...
CREATE INDEX IF NOT EXISTS idx_visit_slots_property_slot ON public.visit_slots(property_id, slot_at);
...
CREATE INDEX IF NOT EXISTS idx_visit_bookings_tenant   ON public.visit_bookings(tenant_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_visit_bookings_property ON public.visit_bookings(property_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_messages_thread           ON public.messages(property_id, sender_id, recipient_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_owner_reviews_owner ON public.owner_reviews(owner_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_search_alerts_user ON public.search_alerts(user_id, created_at DESC);
...
-- VERIFICATION REQUESTS (vérification physique annonces)
...
CREATE UNIQUE INDEX IF NOT EXISTS cgu_singleton ON public.cgu ((true)) WHERE id = 1;
INSERT INTO public.cgu (id, content_fr, content_mg)
VALUES (1, 'Les Conditions Générales d''Utilisation seront publiées prochainement.', 'Ny fitsipika fampiasana dia havoaka tsy ho ela.')
ON CONFLICT DO NOTHING;
...
CREATE TABLE IF NOT EXISTS public.site_settings (
  ...
  hero_subtitle              TEXT NOT NULL DEFAULT 'La première plateforme immobilière locale avec vérification sur site.',
  token_price                BIGINT  NOT NULL DEFAULT 500,
  purchase_instructions      TEXT    NOT NULL DEFAULT 'Effectuez votre paiement par Mvola, Orange Money ou Airtel Money.',
...
DROP TRIGGER IF EXISTS trg_owner_reviews_updated   ON public.owner_reviews;
...
CREATE TRIGGER trg_owner_reviews_updated    BEFORE UPDATE ON public.owner_reviews    FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE TRIGGER property_contacts_updated_at BEFORE UPDATE ON public.property_contacts FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

-- Signup → création automatique du profil
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- Validation contenu annonces
...
-- Anti-escalade de privilèges owner
...
-- Auto-premium si abonnement actif au moment de la création d'annonce
DROP TRIGGER IF EXISTS properties_apply_subscription ON public.properties;
...
DROP POLICY IF EXISTS "Roles: admin insert others"  ON public.user_roles;
DROP POLICY IF EXISTS "Roles: admin update others"  ON public.user_roles;
DROP POLICY IF EXISTS "Roles: admin delete others"  ON public.user_roles;

CREATE POLICY "Roles: select own or admin" ON public.user_roles FOR SELECT
  USING (auth.uid() = user_id OR public.is_admin(auth.uid()));
-- Les admins ne peuvent gérer que les AUTRES (pas leur propre rôle)
CREATE POLICY "Roles: admin insert others" ON public.user_roles FOR INSERT TO authenticated
  WITH CHECK (public.is_admin(auth.uid()) AND user_id <> auth.uid());
CREATE POLICY "Roles: admin update others" ON public.user_roles FOR UPDATE TO authenticated
  USING (public.is_admin(auth.uid()) AND user_id <> auth.uid())
  WITH CHECK (public.is_admin(auth.uid()) AND user_id <> auth.uid());
CREATE POLICY "Roles: admin delete others" ON public.user_roles FOR DELETE TO authenticated
  USING (public.is_admin(auth.uid()) AND user_id <> auth.uid());
...
-- WITH CHECK double-verrou : l'owner ne peut pas toucher is_premium/is_verified
...
DROP POLICY IF EXISTS "Contacts: owner/admin insert" ON public.property_contacts;
DROP POLICY IF EXISTS "Contacts: owner/admin update" ON public.property_contacts;

-- Seuls owner/admin lisent directement. Les locataires passent par get_visit_phone()
CREATE POLICY "Contacts: owner/admin select" ON public.property_contacts FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.properties p
    WHERE p.id = property_contacts.property_id AND (p.owner_id = auth.uid() OR public.is_admin(auth.uid()))));
...
-- ---- property_photos ----
DROP POLICY IF EXISTS "Photos: public read"   ON public.property_photos;
DROP POLICY IF EXISTS "Photos: owner manage"  ON public.property_photos;
...
DROP POLICY IF EXISTS "Views: owner or admin select" ON public.property_views;

CREATE POLICY "Views: anyone insert" ON public.property_views FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.properties p WHERE p.id = property_views.property_id AND p.status = 'active'));
CREATE POLICY "Views: owner or admin select" ON public.property_views FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.properties p WHERE p.id = property_views.property_id AND p.owner_id = auth.uid()) OR public.is_admin(auth.uid()));

-- ---- property_reports ----
DROP POLICY IF EXISTS "Reports: own insert"           ON public.property_reports;
DROP POLICY IF EXISTS "Reports: own or admin select"  ON public.property_reports;
DROP POLICY IF EXISTS "Reports: admin update"         ON public.property_reports;
...
-- ---- favorites ----
DROP POLICY IF EXISTS "Favorites: own only" ON public.favorites;
CREATE POLICY "Favorites: own only" ON public.favorites FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- ---- contact_unlocks ----
DROP POLICY IF EXISTS "Unlocks: own select"     ON public.contact_unlocks;
DROP POLICY IF EXISTS "Unlocks: no client insert" ON public.contact_unlocks;
DROP POLICY IF EXISTS "Unlocks: no client update" ON public.contact_unlocks;
DROP POLICY IF EXISTS "Unlocks: no client delete" ON public.contact_unlocks;

CREATE POLICY "Unlocks: own select"      ON public.contact_unlocks FOR SELECT USING (auth.uid() = user_id OR public.is_admin(auth.uid()));
-- Aucun client ne peut écrire directement — seul le service_role (via serverFn) peut insérer
...
-- ---- visit_slots ----
DROP POLICY IF EXISTS "Slots: public read"   ON public.visit_slots;
DROP POLICY IF EXISTS "Slots: owner manage"  ON public.visit_slots;
...
-- ---- visit_bookings ----
DROP POLICY IF EXISTS "Bookings: tenant select own" ON public.visit_bookings;
DROP POLICY IF EXISTS "Bookings: tenant insert"     ON public.visit_bookings;
...
DROP POLICY IF EXISTS "Reviews: author insert with interaction" ON public.owner_reviews;
DROP POLICY IF EXISTS "Reviews: author update"                 ON public.owner_reviews;
DROP POLICY IF EXISTS "Reviews: author or admin delete"        ON public.owner_reviews;
...
CREATE POLICY "Reviews: author insert with interaction" ON public.owner_reviews FOR INSERT
  WITH CHECK (
    auth.uid() = author_id AND author_id <> owner_id
    AND EXISTS (SELECT 1 FROM public.contact_unlocks cu
      JOIN public.properties p ON p.id = cu.property_id
      WHERE cu.user_id = auth.uid() AND p.owner_id = owner_reviews.owner_id)
  );
CREATE POLICY "Reviews: author update" ON public.owner_reviews FOR UPDATE
  USING (auth.uid() = author_id) WITH CHECK (auth.uid() = author_id);
CREATE POLICY "Reviews: author or admin delete" ON public.owner_reviews FOR DELETE
  USING (auth.uid() = author_id OR public.is_admin(auth.uid()));

-- ---- search_alerts ----
DROP POLICY IF EXISTS "Alerts: own all" ON public.search_alerts;
CREATE POLICY "Alerts: own all" ON public.search_alerts FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- ---- token_transactions ----
DROP POLICY IF EXISTS "Tx: own select"      ON public.token_transactions;
DROP POLICY IF EXISTS "Tx: no client insert" ON public.token_transactions;
DROP POLICY IF EXISTS "Tx: no client update" ON public.token_transactions;
DROP POLICY IF EXISTS "Tx: no client delete" ON public.token_transactions;
...
DROP POLICY IF EXISTS "Purchase: own select"   ON public.token_purchase_requests;
DROP POLICY IF EXISTS "Purchase: own insert"   ON public.token_purchase_requests;
DROP POLICY IF EXISTS "Purchase: admin update" ON public.token_purchase_requests;
...
-- ---- subscriptions ----
DROP POLICY IF EXISTS "Sub: own or admin select" ON public.subscriptions;
DROP POLICY IF EXISTS "Sub: no client insert"    ON public.subscriptions;
DROP POLICY IF EXISTS "Sub: no client update"    ON public.subscriptions;
DROP POLICY IF EXISTS "Sub: no client delete"    ON public.subscriptions;
...
-- ---- kyc_requests ----
DROP POLICY IF EXISTS "KYC: own select"   ON public.kyc_requests;
DROP POLICY IF EXISTS "KYC: own insert"   ON public.kyc_requests;
DROP POLICY IF EXISTS "KYC: admin update" ON public.kyc_requests;
...
DROP POLICY IF EXISTS "Verif: own select"   ON public.verification_requests;
...
-- ---- cgu ----
DROP POLICY IF EXISTS "CGU: public read" ON public.cgu;
DROP POLICY IF EXISTS "CGU: admin update" ON public.cgu;
DROP POLICY IF EXISTS "CGU: no insert"   ON public.cgu;
DROP POLICY IF EXISTS "CGU: no delete"   ON public.cgu;
...
-- ---- user_bans ----
DROP POLICY IF EXISTS "Bans: own select"   ON public.user_bans;
DROP POLICY IF EXISTS "Bans: admin manage" ON public.user_bans;
...
-- ---- admin_logs ----
DROP POLICY IF EXISTS "Logs: admin only" ON public.admin_logs;
...
DO $$ BEGIN
  CREATE POLICY "Service role: email_send_log read"   ON public.email_send_log   FOR SELECT USING (auth.role() = 'service_role');
  CREATE POLICY "Service role: email_send_log insert" ON public.email_send_log   FOR INSERT WITH CHECK (auth.role() = 'service_role');
  CREATE POLICY "Service role: email_send_log update" ON public.email_send_log   FOR UPDATE USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
  CREATE POLICY "Service role: email_send_state"      ON public.email_send_state FOR ALL   USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
  CREATE POLICY "Service role: suppressed read"        ON public.suppressed_emails FOR SELECT USING (auth.role() = 'service_role');
  CREATE POLICY "Service role: suppressed insert"      ON public.suppressed_emails FOR INSERT WITH CHECK (auth.role() = 'service_role');
  CREATE POLICY "Service role: tokens read"            ON public.email_unsubscribe_tokens FOR SELECT USING (auth.role() = 'service_role');
  CREATE POLICY "Service role: tokens insert"          ON public.email_unsubscribe_tokens FOR INSERT WITH CHECK (auth.role() = 'service_role');
  CREATE POLICY "Service role: tokens update"          ON public.email_unsubscribe_tokens FOR UPDATE USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- ---- Realtime channel: messages → participants seulement ----
...
DROP POLICY IF EXISTS "Property photos: owner upload"  ON storage.objects;
...
-- REPLICA IDENTITY FULL pour le diff complet sur ces tables
...
-- ============================================================
-- 9. GRANTS (droits d'exécution des fonctions RPC)
-- ============================================================

-- Helpers RLS : accessibles par anon + authenticated (nécessaires pour les politiques)
GRANT EXECUTE ON FUNCTION public.has_role(UUID, public.app_role) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.is_admin(UUID) TO anon, authenticated, service_role;

-- get_visit_phone : RPC accessible uniquement aux utilisateurs connectés
...
-- Fonctions critiques : service_role uniquement (les serverFn utilisent supabaseAdmin)
...
-- Wrappers email queue : service_role uniquement
...
-- Fonctions trigger-only : inaccessibles en RPC
...
-- ============================================================
-- 10. CRON JOB : nettoyage comptes non confirmés (toutes les 5 min)
-- ============================================================
DO $$
BEGIN
